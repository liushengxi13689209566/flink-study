 ...
 * @author oker
 */
public class EventTimeTimerFunctionTest {
    public static void main(String[] args) throws Exception {
        ...
        streamSource.keyBy(item -> item.getUser()).process(new KeyedProcessFunction<String, Event, String>() {
            ...
            public void processElement(Event event, KeyedProcessFunction<String, Event, String>.Context context, Collector<String> collector) throws Exception {
                ...
                collector.collect(context.getCurrentKey() + " 数据到达，时间戳是：" + new Timestamp(event.getTimestamp()) + " waterMark 是：" + context.timerService().currentWatermark());
                // 注册一个10秒后的定时器
            ...
            public void onTimer(long timestamp, KeyedProcessFunction<String, Event, String>.OnTimerContext ctx, Collector<String> out) throws Exception {
                out.collect(ctx.getCurrentKey() + " 定时器触发，触发时间是：" + new Timestamp(timestamp) + " waterMark是" + ctx.timerService().currentWatermark());
            }
